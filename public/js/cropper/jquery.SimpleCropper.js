...
(function($) {
  ...
  $.fn.simpleCropper = function() {
    ...
    var jcrop_api;
      var gallery_images=[];
    var bottom_html = "<canvas id='myCanvas' style='display:none;'></canvas><div id='modal'></div><div id='preview'><div class='buttons'><div id='cancel' class='cancel'></div><div class='ok' id='ok'></div></div></div>";
    ...
    $(document).ready(function() {
      ...
      $('#newUserAvatar').change(function(click) {
    ...
    function imageUpload(dropbox) {
      var file = $("#newUserAvatar").get(0).files[0];
      //var file = document.getElementById('newUserAvatar').files[0];
      var imageType = /image.*/;
    ...
    function preview() {
      ...
      file_display_area.css({'background-image':'url('+dataUrl+')','background-size':'cover'})
        $('#form_propertyOgImage').val(dataUrl);
    }
